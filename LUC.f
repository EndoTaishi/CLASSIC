...
C     IDAY      - DAY OF YEAR
C     TODFRAC   - TODAY'S FRACTIONAL COVERAGE OF ALL PFTs 
C     YESFRAC   - YESTERDAY'S FRACTIONAL COVERAGE OF ALL PFTs 
C     INTERPOL  - IF TODFRAC & YESFRAC ARE PROVIDED THEN INTERPOL MUST
C                 BE SET TO 1 SO THAT THIS SUBROUTINE DOESN'T DO ITS
C                 OWN INTERPOLATION USING PFCANCMX AND NFCANCMX WHICH
C                 ARE YEAR END VALUES     
C     UPDATES
C
...
C                u-MOL CO2/M2.SEC
C     LUCLTRIN - LUC RELATED INPUT TO LITTER POOL, u-MOL CO2/M2.SEC
C     LUCSOCIN - LUC RELATED INPUT TO SOIL CARBON POOL, u-MOL CO2/M2.SEC
C
C     ----------------------------------------------------------------    
C
      IMPLICIT NONE
...
100   CONTINUE
C
C     FIND/USE PROVIDED CURRENT AND PREVIOUS DAY'S FRACTIONAL COVERAGE 
C
      IF(INTERPOL.EQ.0) THEN ! PERFORM INTERPOLATION 
       DO 110 J = 1, ICC
        DO 111 I = IL1, IL2
          DELFRAC(I,J)=NFCANCMX(I,J)-PFCANCMX(I,J) !CHANGE IN FRACTION
          DELFRAC(I,J)=DELFRAC(I,J)/365.0
CRL L          FCANCMX(I,J)=PFCANCMX(I,J)+(FLOAT(IDAY)*DELFRAC(I,J)) !  CURRENT DAY
CRL L          FCANCMY(I,J)=PFCANCMX(I,J)+(FLOAT(IDAY-1)*DELFRAC(I,J)) ! PREVIOUS DAY
...
111     CONTINUE
110    CONTINUE
      ELSE IF(INTERPOL.EQ.1) THEN ! USE PROVIDED VALUES BUT STILL CHECK
C                                   THEY ARE NOT -VE
       DO 115 J = 1, ICC
        DO 116 I = IL1, IL2
...
C
     ...
     &                (TERM*LITRMASS(I,J)*GRCLAREA(I)*1.0E+06)
          TOTDMAS2(I)=TOTDMAS2(I)+
     &                (TERM*SOILCMAS(I,J)*GRCLAREA(I)*1.0E+06)
341     CONTINUE
340   CONTINUE
C
      DO 350 I = IL1, IL2
...
C
     ...
     &                     FCANCMY(I,M)-FCANCMX(I,M)
              ...
              ENDIF
C
C             ROOTMASS NEEDS TO BE CHOPPED AS WELL AND ALL OF IT GOES TO
C             THE LITTER/PAPER POOL
C
              REDUBMAS2=0.0
              REDUBMAS2=(FCANCMY(I,M)-FCANCMX(I,M))*GRCLAREA(I)
     &                  *ROOTMASS(I,M)*1.0E+06 ! 1.0E+06 CHANGES KM2 -> M2
C
C
C             KEEP ADDING CHOPPED OFF BIOMASS FOR EACH PFT TO GET THE TOTAL
C             FOR A GRID CELL FOR DIAGNOSTICS
C
...
630   CONTINUE
C
C     CALCULATE IF THE CHOPPED OFF BIOMASS EQUALS THE SUM OF GRSUMCOM(I),
C     GRSUMPAP(I) & GRSUMFUR(I)
C
      DO 632 I = IL1, IL2
...
C                                   ! WILL BE EMITTED 
C       LUCLTRIN(I)=GRDENPAP(I)+GRDENLIT(I) ! FLUX IN KG C/M2.DAY
C       LUCSOCIN(I)=GRDENFUR(I)+GRDENSOC(I) ! FLUX IN KG C/M2.DAY
C
        LUCLTRIN(I)=GRDENPAP(I) ! FLUX IN KG C/M2.DAY
        LUCSOCIN(I)=GRDENFUR(I) ! FLUX IN KG C/M2.DAY
C
C       CONVERT ALL LAND USE CHANGE FLUXES TO u-MOL CO2-C/M2.SEC
...
720   CONTINUE
C
C     TOTAL LITTER MASS (BEFORE + INPUT FROM CHOPPED OFF BIOMASS)
C     AND AFTER MUST BE SAME
C
      DO 722 I = IL1, IL2
