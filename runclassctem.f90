...
! #ED - this subroutine has the basics that will go into the MPI driver:
      ...
      ! #ED - This subroutine below opens the MET driving netcdf. In the future, we
      ! need to figure out whether this reads in 1 year at a time or what.
...
!     Initialize the CTEM parameters
    ...
    ! #ED - Since the MPI driver tells this driver only that it needs to run at
    ! gricell XXYY then we need only allocate arrays for nlat =1, nmos = number of
    ! tiles in this particular gridcell, and ignd= number of soil layers (determined
    ! from the initial conditions NC file.
