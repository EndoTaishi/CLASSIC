...
C
     ...
     5        NCYEAR,CO2YR,POPYR,NUMMETCYLYRS,METCYLYRST,
     6        METCYCENDYR,CLIMIYEAR,POPCYCLEYR,CYPOPYR,
     7        LUCYR,CYLUCYR 
C
...
C          READ(70,*) (LITRMASSROW(I,M,J),J=1,ICC)
C          READ(70,*) (SOILCMASROW(I,M,J),J=1,ICC)
          READ(70,*) (LFSTATUSROW(I,M,J),J=1,ICC)
          READ(70,*) (PANDAYSROW(I,M,J),J=1,ICC)

C INITIALIZATION
C          LITRMASSROW(I,M,10)=0.0
C          SOILCMASROW(I,M,10)=0.0
C
72      CONTINUE
...
C     READ IN FROM THE .INI AND .CTM FILES. JM.
...
C
C     ADD FOR COMPETE BY PENG \\
C     NET RADIATION AND PRECIPITATION ESTIMATES FOR CTEM CALCULATION IN BIOCLIM
C
...
C6300  FORMAT(1X,I4,I5,3F9.2,F8.2,F10.2,E12.3,2F12.3,F8.3,A5,I2))
...
C6601  FORMAT(1X,I2,I3,I5,I6,5(F7.2,2F6.3),10F9.4,2(A5,I2))
...
C
            DO J = 1, ICC
C         FLAG! THIS IS WHAT WAS IN CLASS36CTEM CODE:
C              ANNPPVEG_M(I,M,J)=ANNPPVEG_M(I,M,J)*1.0368 ! CONVERT TO gC/M2,YEAR
C              ANNGPPVEG_M(I,M,J)=ANNGPPVEG_M(I,M,J)*1.0368 ! CONVERT TO gC/M2,YEAR
C             ANNNEPVEG_M(I,M,J)=ANNNEPVEG_M(I,M,J)*1.0368 ! CONVERT TO gC/M2,YEAR
C         AND THIS WAS IN YIRAN'S CODE: JM
...
C     &                     SOILCMASROW(I,M,ICCP1)*BAREFRAC
        ...
        TOTCMASS_MN(I)=VGBIOMAS_MN(I)+LITRMASS_MN(I)+SOILCMAS_MN(I)
...
C
       IF(IDAY.EQ.MDAY(NT+1))THEN
        ...
        AVGMNNBP_MN(I)=AVGMNNBP_MN(I)+AVGMNNBP_MN_M(I,M)*FAREROW(I,M)
C
        HETRORES_MN(I)=HETRORES_MN(I)+HETRORES_MN_M(I,M)*FAREROW(I,M)
        AUTORES_MN(I) =AUTORES_MN(I) +AUTORES_MN_M(I,M) *FAREROW(I,M)
        LITRES_MN(I)  =LITRES_MN(I)  +LITRES_MN_M(I,M)  *FAREROW(I,M)
        SOILRES_MN(I) =SOILRES_MN(I) +SOILRES_MN_M(I,M) *FAREROW(I,M)
C
C
...
C     FROM YIRAN'S CODE:
C       DO 884 J=1,ICC          
C        IF (AILCGROW(I,M,J).GT.LAIMAXGVEG_YR_M(I,M,J)) THEN
C         LAIMAXGVEG_YR_M(I,M,J)=AILCGROW(I,M,J)
C        ENDIF 
C884    CONTINUE
C     TO HERE>>
...
C
        ...
        AVGYRNBP_YR(I)=AVGYRNBP_YR(I)+AVGYRNBP_YR_M(I,M)*FAREROW(I,M)
   
        HETRORES_YR(I)=HETRORES_YR(I)+HETRORES_YR_M(I,M)*FAREROW(I,M) 
        AUTORES_YR(I) =AUTORES_YR(I) +AUTORES_YR_M(I,M)*FAREROW(I,M)
        LITRES_YR(I)  =LITRES_YR(I)  +LITRES_YR_M(I,M)*FAREROW(I,M)
        SOILRES_YR(I) =SOILRES_YR(I) +SOILRES_YR_M(I,M)*FAREROW(I,M) 
