...
C      CHECK THE NUMBER OF MOSAICS THAT CAME FROM THE .INI FILE
       IF (NMTEST .NE. NMOS .AND. MOSAIC) THEN
...
C      PLUS WORK ON COMPETITION IS STILL IN PROGRESS
C
C      INITALIZE TO ZERO
       FCANROW=0.0
       DVDFCANROW=0.0
...
113   CONTINUE
C
C     CHANGED THIS SO IT ALSO WILL ONLY CYCLE ON 
C     ONE YEAR OF LUC DATA WHEN NEEDED. SO WHEN CYCLING MET
C     YOU WILL CYCLE ON ONE YEAR OF BOTH POPD AND LUC FRACTIONS.
C
...
C       GET FIRST YEAR OF LUC DATA
        DO I = 1, NLTEST
         ...
         ELSE
          ...
          DO M = 1, NMTEST-1
           J = M
           PFCANCMXROW(I,M,J) = TEMPARRAY(M) !FLAG CORRECT TO SKIP NMTEST CUZ BARE? JM DEC6
          ENDDO !M LOOP
        ENDDO !NLTEST
C
...
C
        ...
        ENDDO
C
      ENDIF ! IF (LNDUSEON .AND. (LOPCOUNT .EQ. 1 .OR. .NOT. CYCLEMET))
C
...
C
       IF(POPDON) THEN
C
C      FOLLOWING PROVIDES THE ABILITY TO HAVE CONSTANT POPN FOR 
...
C
      IF (CTEM2) THEN
        DO 700 I = 1, NML
C
C         COMPETITION RELATED VRIABLES ADDED BY Y. PENG \\
...
C          ASSIGN THE SAME VALUES (ROW) TO GAT  FLAG
        ...
        ENDIF ! IF ((LNDUSEON).AND.(IDAY.EQ.1))
        write(*,*)mosaic
