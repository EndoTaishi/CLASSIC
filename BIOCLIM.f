...
C                     SCALE.
C
C                     NOTE THAT THIS SUBROUTINE IS ONLY NECESSARY WHEN
C                     COMPETITION IS SWITCHED ON.
C     INPUTS 
C
...
C         MONTH AND CURRENT MONTHS WATER SURPLUS/DEFICIT TO ZERO
          IF(IDAY.EQ.(MONTHEND(K-1)+1))THEN
...
C       E-FOLDING SENSE
        DO 280 I = IL1, IL2
          ...
          IF(.NOT. INIBOCLM)THEN
            TWARMM(I)=TWCURYR(I)
            TCOLDM(I)=TCCURYR(I)
            write(*,'(a5,f12.3,i4)')'set',tcoldm(i),I
            GDD5(I)=GDD5CUR(I)
            ARIDITY(I)=ARIDCUR(I)
