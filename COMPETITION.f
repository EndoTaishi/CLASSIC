...
C     J. MELTON       PFTS
C
C     1   OCT 2012  - UPDATE SUBROUTINE AND IMPLEMENT FOR RUNNING WITH
C     Y. PENG         MOSAIC VERSION OF CLASS 3.6
C
...
C
      ...
      INTEGER NUMCROPS
      PARAMETER (NUMCROPS=2)   ! NUMBER OF CROP PFTS
      INTEGER NUMTREEPFTS
      PARAMETER (NUMTREEPFTS=5) ! NUMBER OF TREE PFTS
      INTEGER NUMGRASS
      PARAMETER (NUMGRASS=2)   ! NUMBER OF GRASS PFTS

      PARAMETER (KK=12)  ! PRODUCT OF CLASS PFTs AND L2MAX

      INTEGER GRASS_IND(2)! INDEX OF THE GRASS PFTs (ONLY 2 GRASS PFTS AT PRESENT)
...
C
      DO 330 J = 1, ICC-NUMCROPS   ! J NOW GOES FROM 1 TO ICC-NUMCROPS
        ...
        ENDIF 
        DO 340 I = IL1, IL2
          FRAC(I,J)=MAX(SEEDFRAC(SDFRACIN), !*REAL(PFTEXIST(I,N)), !remove pftextist?
     &              FCANCMX(I,N))
          EXIST1(I,J)=PFTEXIST(I,N)
          C1(I,J)=C(I,N)*REAL(PFTEXIST(I,N))  
          M1(I,J)=M(I,N)
340     CONTINUE
330   CONTINUE
C
...
C
     ...
     &      N
          ...
          ENDIF
          USEFRAC(I,N)=MAX(SEEDFRAC(SDFRACIN), !*REAL(USEEXIST(I,N)), !remove the useexist?
     &                     USEFRAC(I,N))
510     CONTINUE
500   CONTINUE
C
...
C
      DO 540 N = 1, ICC-NUMCROPS
        DO 541 I = IL1, IL2
          ...
          IF(ABS(USEFRAC(I,N)-SEEDFRAC(SDFRACIN)).LE.ZERO) !remove the useexist?
C     &              *REAL(USEEXIST(I,N)).LE.ZERO)
     &    THEN
...
C
     ...
     &    ABS(USEFRAC(I,N)-SEEDFRAC(SDFRACIN)).GT.ZERO)! *  !remove the useexist?
C     &    REAL(USEEXIST(I,N))).GT.ZERO)
     &    THEN
...
C     CORRESPONDING TO THE ACTUAL NUMBER OF PFTs
C
...
590   CONTINUE
C
C       ---> FROM HERE ON WE GET BACK TO OUR USUAL ICC PFTs <----
C
C     GET BARE FRACTION
C
      DO 600 J = 1, ICC
