...
C     * INTEGER CONSTANTS.
C
      INTEGER,PARAMETER :: NLAT=3,NMOS=8,ILG=NLAT*NMOS,NMON=12
...
C
     ...
     5        CYCLEMET,METCYCENDYR,CLIMIYEAR
C
...
C
     ...
     5           SPINFAST,  CO2ON,   LNDUSEON,    MONTH1, 
     6           MONTH2,    XDAY,    POPDON
...
C      * YEARLY OUTPUT FOR CTEM GRID-MEAN
     ...
     5      LAIMAXGVEG_YR_M(NLAT,NMOS,ICC)            
C
...
C
C     JM --PLACE ENTRY TO NAMELIST ROUTINE HERE
...
C
C	These are read in by ARGBUFF now.
C     ------------------------------------------------------
C      CTEMLOOP = 10 ! NO. OF TIMES THE .MET FILE IS TO BE READ. THIS
C                    ! OPTION IS USEFUL TO SEE HOW CTEM's C POOLS
C                    ! EQUILIBRATE WHEN DRIVEN WITH SAME CLIMATE DATA
C                    ! OVER AND OVER AGAIN.
C
C      CTEM1=.FALSE.   ! SET THIS TO TRUE FOR USING STOMATAL CONDUCTANCE
...
C      CTEM2=.FALSE.   ! SET THIS TO TRUE FOR USING CTEM SIMULATED DYNAMIC
...
C
C     JK=5 (all pfts now outputted) CHOOSE CTEM PFT FOR WHICH WE WANT TO WRITE
...
C                    ! PFTs FROM A FILE.
C
      SPINFAST = 1   ! SET THIS TO A HIGHER NUMBER UP TO 10 TO SPIN UP
C                    ! SOIL CARBON POOL FASTER

C     >>>>>JM EDIT
      CYCLEMET = 1   ! TO CYCLE OVER ONLY A FIXED NUMBER OF YEARS 
C		     ! (NUMMETCYLYRS) STARTING AT A CERTAIN YEAR (METCYLYRST)
C                    ! PUT THE CYCLEMET FLAG TO 1 OR NOT (0), IF CYCLEMET = 1
C		     ! THEN PUT CO2ON = 0 AND SET AN APPOPRIATE SETCO2CONC, ALSO
C                    ! IF POPDON IS SET TO 1, IT WILL CHOOSE THE POPN DATA FOR YEAR
C		     ! METCYLYRST AND CYCLE ON THAT.
C
      NUMMETCYLYRS = 2 ! YEARS OF THE CLIMATE FILE TO SPIN UP ON REPEATEDLY
C		       ! IGNORED IF CYCLEMET = 0
C
      METCYLYRST = 1905 ! CLIMATE YEAR TO START THE SPIN UP ON
C		        ! IGNORED IF CYCLEMET = 0
C     <<<<<MJ EDIT
C
      LOPCOUNT = 1   ! INITIALIZE LOOP COUNT TO 1.
C
      CO2ON = 0      ! USE CO2 TIME SERIES (1) OR NOT (0), SET TO 0 IF CYCLEMET = 1
C
      SETCO2CONC = 365.00  ! SET THE VALUE OF ATMOSPHERIC CO2 IF CO2ON IS SET TO 0.
C
      POPDON = 0     ! USE POPULATION DENSITY DATA TO CALCULATE FIRE EXTINGUISHING 
C                    ! PROBABILITY AND PROBABILITY OF FIRE DUE TO HUMAN CAUSES (1) 
C                    ! OR READ DIRECTLY FROM .CTM FILE (0)
C
      PARALLELRUN = .FALSE. 
C                    ! SET THIS TO BE TRUE IF MODEL IS RUN IN PARALLEL MODE FOR 
C                    ! MULTIPLE GRID CELLS, OUTPUT IS LIMITED TO MONTHLY & YEARLY 
C                    ! GRID-MEAN ONLY. ELSE THE RUN IS IN STAND ALONE MODE, IN WHICH 
C                    ! OUTPUT INCLUDES HALF-HOURLY AND DAILY AND MOSAIC-MEAN AS WELL.
C
      COMPETE=.FALSE. 
C                    ! SET THIS TO TRUE IF COMPETITION BETWEEN PFTs IS
C                    ! TO BE IMPLIMENTED
C
      RSFILE=.FALSE.
C                    ! SET THIS TO TRUE IF RESTART FILES (.INI_RS AND .CTM_RS)   
C                    ! ARE WRITTEN AT THE END OF EACH YEAR. THESE FILES ARE  
C                    ! NECESSARY FOR CHECKING WHETHER THE MODEL REACHES 
C                    ! EQUILIBRIUM AFTER RUNNING FOR A CERTAIN YEARS. 
C                    ! SET THIS TO FALSE IF RESTART FILES ARE NOT NEEDED 
C                    ! (KNOWN HOW MANY YEARS THE MODEL WILL RUN)
C
...
C
      ARGCOUNT = IARGC()
       ...
       IF(ARGCOUNT.NE.3)THEN
         ...
         WRITE(*,*)' '
         WRITE(*,*)'RUNCLASS35CTEM SITE_NAME CTEMON NLOOP NCYEAR'
         WRITE(*,*)' '
         WRITE(*,*)'- SITE_NAME refers to the prefix of file names'
         WRITE(*,*)' '
         WRITE(*,*)'- CTEMON is T for coupling with CTEM '
         WRITE(*,*)'            F for running CLASS only '
         WRITE(*,*)' '
         WRITE(*,*)'- NLOOP refers to the # of loop to read .MET file '
         STOP
      END IF
C
      CALL GETARG(3, ARGBUFF)
      READ (ARGBUFF, *) CTEMLOOP
...
6125  FORMAT('  YEAR   LAIMAXG  VGBIOMAS  STEMMASS  ROOTMASS  LITRMASS' 
     &'  SOILCMAS  TOTCMASS  ANNUALNPP ANNUALGPP ANNUALNEP')
6225  FORMAT('          m2/m2   Kg C/m2   Kg C/m2   Kg C/m2    Kg C/m2'
     &'  Kg C/m2   Kg C/m2   gC/m2.yr  gC/m2.yr  gC/m2.yr  ')
C
...
C     >>>>>JM EDIT
      IF (CYCLEMET .EQ. 1 .AND. IYEAR .LT. METCYLYRST) THEN !NOT AT START YEAR
         GO TO 225  !DONT RUN MODEL, GET ANOTHER TIMESTEP OF DATA
      ELSE IF (CYCLEMET .EQ. 1 .AND. IYEAR .GT. METCYCENDYR) THEN
...
300   CONTINUE
C
      IF (CO2ON.EQ.0) THEN  !SPECIFY CO2 CONCENTRATION FOR PHTSYN
       DO I=1,NLTEST
        DO M=1,NMTEST
...
C     A RUN WITH CYCLING MET. 
      ...
      IF (CYCLEMET .EQ. 1 .AND. IYEAR .NE. METCYLYRST) THEN     
C     MET IS CYCLING AND WE ALREADY HAVE THE POPN DATA FOR FIRST
C     YEAR SO WE WILL JUST KEEP CYCLING OVER THAT
         GO TO 325
      END IF
...
C
     ...
     Q      FAREGAT,     GAVGSCMSGAT, RMLVEGACCGAT, RMSVEGGAT,
     &      RMRVEGGAT,   RGVEGGAT,    VGBIOMAS_VEGGAT,
...
C
     ...
     L      FAREROW,     GAVGSCMSROW, RMLVEGACCROW, RMSVEGROW,
     &      RMRVEGROW,   RGVEGROW,    VGBIOMAS_VEGROW,
...
950       CONTINUE
C
8700        FORMAT(1X,I5,9F10.3,2(A5,I1))
...
C
      ...
      ENDIF !CTEM2  
C
8104  FORMAT(1X,I4,I5,7F10.3,2(A5,I1))
8105  FORMAT(1X,I5,10F10.3,2(A5,I1))
C
C===================== CTEM =====================================/
C=======================================================================
C
