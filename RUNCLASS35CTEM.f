...
C
     ...
     A     RMLVEGROW(NLAT,NMOS,ICC),   RHGRD(NLAT),
...
C                    ! PFTs FROM A FILE.
C
      CTEMLOOP = 10  ! NO. OF TIMES THE .MET FILE IS TO BE READ. THIS
C                    ! OPTION IS USEFUL TO SEE HOW CTEM's C POOLS
C                    ! EQUILIBRATE WHEN DRIVEN WITH SAME CLIMATE DATA
C                    ! OVER AND OVER AGAIN.
C
      NCYEAR = 32    ! NO. OF YEARS OF THE .MET FILE. FOR PREVIOUS TEST RUNS,
...
C
C     JK=5           ! CHOOSE CTEM PFT FOR WHICH WE WANT TO WRITE
C                    ! SOME SELECTED RESULTS
C
      LOPCOUNT = 1   ! INITIALIZE LOOP COUNT TO 1.
C
      CO2ON = 0      ! USE CO2 TIME SERIES (1) OR NOT (0)
C
      POPDON = 1     ! USE POPULATION DENSITY DATA TO CALCULATE FIRE EXTINGUISHING 
...
C
      ARGCOUNT = IARGC()
       ...
       IF(ARGCOUNT.NE.3)THEN
         ...
         WRITE(*,*)' '
         WRITE(*,*)'RUNCLASS35CTEM SITE_NAME CTEMON NLOOP'
         WRITE(*,*)' '
...
C
      IF (CO2ON.EQ.0) THEN  !SPECIFY CO2 CONCENTRATION FOR PHTSYN
       DO I=1,NLTEST
        DO M=1,NMTEST
c         CO2CONCROW(I,M)=320.00
         CO2CONCROW(I,M)=285.00
        ENDDO
       ENDDO
      ENDIF
C
...
C
     ...
     6      RMLCGVEGGAT, CANRESGAT,   RHGAT,        SDEPGAT,
...
C
     ...
     1      RMLCGVEGROW, CANRESROW,   RHGRD,        SDEPROW,
...
C    ------------ CTEM INPUTS ABOVE THIS LINE, OUTPUTS BELOW -----------|
     ...
     U  RHGAT,SANDGAT,CLAYGAT,ORGMGAT,PRESGAT )
C
C===================== CTEM =====================================/
C-----------------------------------------------------------------------
...
C===================== CTEM =====================================\
     ...
     6      RMLCGVEGROW, CANRESROW,   RHGRD,        SDEPROW,
...
C
     ...
     1      RMLCGVEGGAT, CANRESGAT,   RHGAT,        SDEPGAT,
...
C===================== CTEM =====================================\
      ...
      IF(LOPCOUNT.LE.CTEMLOOP)THEN
        CLOSE(51)
      OPEN(UNIT=51,FILE=ARGBUFF(1:STRLEN(ARGBUFF))//'.MET',STATUS='OLD')
       IF(POPDON.EQ.1) THEN
        CLOSE(91)
      OPEN(UNIT=91,FILE=ARGBUFF(1:STRLEN(ARGBUFF))//'.POPD',
     &     STATUS='OLD')
