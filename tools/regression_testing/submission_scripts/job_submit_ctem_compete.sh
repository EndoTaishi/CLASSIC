...
output_directory=/space/hall1/sitestore/eccc/crd/ccrp/crp102/checksum_testing/ctem_compete
...
job_options_file=$CI_PROJECT_DIR/tools/regression_testing/submission_scripts/job_options_ctem_compete.txt
...
case "$HOSTID" in
  xc*)   platform=xc
         phi=26000           # gridcell-years/hour metric for hare/brooks
         max_wallclock=3 ;;
  *ppp*) platform=ppp
...
# the same node. On ppp1/ppp2, the hosts are numbered sequentially from zero and are listed in the $RUMPIRUN_HOSTFILE at runtime.
...
if [ "$projectedGrid" = '.true.' ] ; then
  ...
  do
    ...
    if [ $platform = ppp ] ; then
      command_lines+="time rumpirun -H $i -n 44 -npernode 44 run_model_script $((i+1)) $col1 $col2 $((row_boundaries[$i]+1)) ${row_boundaries[$((i+1))]} &\n"
    else
      command_lines+="time aprun -n 36 -N 36 run_model_script $((i+1)) $col1 $col2 $((row_boundaries[$i]+1)) ${row_boundaries[$((i+1))]} &\n"
    fi
  done
  ...
  do
    ...
    if [ $platform = ppp ] ; then
      command_lines+="time rumpirun -H $i -n 44 -npernode 44 run_model_script $((i+1)) $lon1 $lon2 ${boundaries[$i]} ${boundaries[$((i+1))]} &\n"
    else
      command_lines+="time aprun -n 36 -N 36 run_model_script $((i+1)) $lon1 $lon2 ${boundaries[$i]} ${boundaries[$((i+1))]} &\n"
    fi
