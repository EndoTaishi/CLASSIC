...
c	 inputs-----------------------------------------------------------
	...
	1         thliq(ilg,ignd),	thpor(ilg,ignd), 	bi(ilg,ignd), 
	2	     zbotw(ilg,ignd),	delzw(ilg,ignd),	frac(ilg), tfrez
	3         ,litrmassms(ilg)	
...
c	parameters (from PCARs) can be moved to ctem_params----------------
	 ...
	 real:: 	bsrateltms = 0.20	!yr-1, 0.05 (T.Moore unpubished data)
							...
							!0,5 for ctem. So scale up the value
							!to be comparable to ctem values
c	------------------------------------------------------------------
c
c	initialization
	 do 10 i = il1, il2
...
c
	 do 20 	i= il1, il2
          ewtable(i)= wtable(i)         !only for MBBOG 
...
c		
	...
	1				     ewtable(i)+0.01
		        ratescpa(i) = -0.01*exp(-40*ewtable(i))-1.12*exp(-25*
	1			          hpd(i))+0.015*ewtable(i)+0.005119
	   	    elseif(ewtable(i) .gt. 0.30) 	                    then
		...
		do 60 i = il1, il2
             if(isand(i,j).eq.-3.or.isand(i,j).eq.-4) then  !ice or rock
	      	psi(i,j) = 10000.0 ! a large number so that ltrmoscl = 0.2
           	...
           	psi(i,j)=psisat(i,j)*(thliq(i,j)/thpor(i,j))**(-bi(i,j))
		   endif 
60	 continue	 
...
c    min moisture factor is at 0.5 for moss litter but at 0.2 for soil). 
	...
	1				/(log10(10000.0)-log10(6.0)) )
         	elseif (litpsims(i).le.6.0 .and. litpsims(i) .ge. 4.0) then
              	ltrmosclms(i)=1.0
        	elseif (litpsims(i).lt.4.0 .and. litpsims(i).gt.psisat(i,1)) 
	1												then 
              	ltrmosclms(i)=1.0-0.2*((log10(4.0)-log10(litpsims(i)))/   
