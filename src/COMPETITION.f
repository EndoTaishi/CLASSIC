...
C
      INTEGER N, K, INIRANK(7), T1(ILG), RANK(ILG,ICC-2), K1, K2, L,
...
C     THEREFORE WE ONLY HAVE 7 RANKS CORRESPONDING TO THE REMAINING 7
C     PFTs. THE FIRST 5 ARE TREE PFTs AND THE LAST TWO ARE THE C3 AND C4
C     GRASSES.

      DATA INIRANK/1, 2, 3, 4, 5, 6, 7/
C
C
...
C
      ...
      ELSE IF(BOER.AND. (.NOT.LOTVOL). AND. (.NOT.ARORA))THEN
        ...
        SDFRACIN=2   ! SEED FRACTION INDEX
      ELSE
        WRITE(6,*)'CHOOSE COMPETITION MODEL PROPERLY'
...
C
      DO 1220 J = 1, ICC
       IF(J.NE.6.AND.J.NE.7)THEN  ! DO NOT RUN FOR CROPS
        DO 1230 I = IL1, IL2
...
200   CONTINUE
C
C        ---> FROM HERE ON WE ASSUME THAT WE ONLY HAVE 7 PFTs <----
...
C     REMAINING 7 PFTs. IN ADDITION PFTs WHICH SHOULDN'T EXIST IN THE
...
C
      DO 220 I = IL1, IL2
        CROPFRAC(I)=FCANCMX(I,6)+FCANCMX(I,7)
220   CONTINUE
C
...
C
      DO 310 I = IL1, IL2
        IF(USENPPVG(I,8).GE.USENPPVG(I,9))THEN
          RANK(I,6)=6
          RANK(I,7)=7
        ELSE IF(USENPPVG(I,8).LT.USENPPVG(I,9))THEN
          RANK(I,6)=7
          RANK(I,7)=6
        ENDIF
310   CONTINUE
...
C     ALL.
C
      DO 330 J = 1, ICC-2   ! J NOW GOES FROM 1 TO 7
        IF(J.LE.5)THEN
          N=J
        ELSE
          N=J+2
...
C
      DO 350 N = 1, ICC-2   ! N NOW GOES FROM 1 TO 7
        DO 360 I = IL1, IL2
...
350   CONTINUE
C
      DO 400 N = 1, ICC-2   ! N NOW GOES FROM 1 TO 7
        DO 410 I = IL1, IL2
C
...
C     CORRESPONDING TO THE ACTUAL 9 PFTs
C
      DO 590 J = 1, ICC-2   ! J NOW GOES FROM 1 TO 7
        DO 591 I = IL1, IL2
C
          IF(RANK(I,J).LE.5)THEN
            K=RANK(I,J)
          ELSE
            K=RANK(I,J)+2
          ENDIF 
...
C
      ...
      DO 660 J = 1, ICC
       IF(J.NE.6.OR.J.NE.7)THEN  ! DO NOT RUN FOR CROPS
        DO 661 I = IL1, IL2
C
