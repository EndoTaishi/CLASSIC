...
C    NEW FIRE EMISSIONS/AEROSOLS
     ...
     W                    EMIT_TPM, EMIT_TC,  EMIT_OC,  EMIT_BC,
...
C                 INITIALIZED FROM SCRATCH (=0) OR BEING INITIALIZED
C                 FROM SOME SPUN UP VALUES(=1).
...
C
      INTEGER       INIBOCLM,   PFTEXIST(ILG,ICC)
C
      LOGICAL COMPETE
C
...
C
        DO 81 I = IL1, IL2
          GRCLAREA(I) = 4*PI*(EARTHRAD**2)*WL(CURLATNO(I))*ML(I)
...
C
     ...
     &                  SPINFAST * (HUTRSTEP(I,J) -  SCRESTEP(I,J)) 
C
...
C     IS 1 HECTARE = 10,000 M2. THE DISTURBANCE SUBROUTINE MAY BE CALL EXITPED 
