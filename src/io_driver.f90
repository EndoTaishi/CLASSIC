...
contains
    ...
    subroutine class_hh_w(lonLocalIndex,latLocalIndex,nltest,nmtest,ncount,iday,realyr,SBC,DELT,TFREZ)!,FAREROT,jdstd,jdsty,jdendd,jdendy)
...
600     CONTINUE
        ...
        timeStamp = (realyr - 1 - refyr) !* !FLAG sort this out.
        ...
        DO I=1,NLTEST
            ...
            call writeOutput1D(lonLocalIndex,latLocalIndex,'snomlt_hh' ,timeStamp,'snomlt', [HMFNROW(I)])
            ...
            ELSE
                ZSN=0.0
            ENDIF
            call writeOutput1D(lonLocalIndex,latLocalIndex,'zsnow_hh'   ,timeStamp,'snodep', [ZSN])
            ...
            ELSE
                TCN=0.0
            ENDIF
            call writeOutput1D(lonLocalIndex,latLocalIndex,'tcan_hh'   ,timeStamp,'tcanopy', [TCN])
            ...
            call writeOutput1D(lonLocalIndex,latLocalIndex,'taacc_hh'  ,timeStamp,'ts', [TSURF])
            ...
            ELSE
                TSN=0.0
            ENDIF
            call writeOutput1D(lonLocalIndex,latLocalIndex,'tsno_hh'   ,timeStamp,'tsnow', [TSN])
            ...
            call writeOutput1D(lonLocalIndex,latLocalIndex,'gt_hh'   ,timeStamp,'gt', [GTROW(I)-TFREZ])
            call writeOutput1D(lonLocalIndex,latLocalIndex,'snoacc_hh' ,timeStamp,'snw', [SNOROW(I)])
            call writeOutput1D(lonLocalIndex,latLocalIndex,'wsnoacc_hh',timeStamp,'wsnw', [WSNOROW(I)])
            call writeOutput1D(lonLocalIndex,latLocalIndex,'snodens_hh',timeStamp,'snwdens', [RHOSROW(I)])

            call writeOutput1D(lonLocalIndex,latLocalIndex,'rofacc_hh',timeStamp,'mrro', [ROFROW(I)])
            call writeOutput1D(lonLocalIndex,latLocalIndex,'rofo_hh',timeStamp,'mrroo', [ROFOROW(I)]) !name?
            call writeOutput1D(lonLocalIndex,latLocalIndex,'rofs_hh',timeStamp,'mrros', [ROFSROW(I)]) !name?
            call writeOutput1D(lonLocalIndex,latLocalIndex,'rofb_hh',timeStamp,'mrrob', [ROFBROW(I)]) !name?
            ...
            if (dopertileoutput) then
                do m = 1, nmtest
                    ...
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'snomlt_hh_t'   ,timeStamp,'snomlt', [HMFNROT(I,M)])
                    ...
                    ELSE
                        ZSN=0.0
                    ENDIF
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'zsnow_hh_t'   ,timeStamp,'snodep', [ZSN])
                    ...
                    ELSE
                        TCN=0.0
                    ENDIF
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'tcan_hh_t'   ,timeStamp,'tcanopy', [TCN])
                    ...
                    ELSE
                        TSN=0.0
                    ENDIF
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'tsno_hh_t'   ,timeStamp,'tsnow', [TSN])
                    ...
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'gt_hh_t'   ,timeStamp,'gt', [GTROT(I,M)-TFREZ])
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'snoacc_hh_t' ,timeStamp,'snw', [SNOROT(I,M)])
                    ...
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'rofacc_hh_t',timeStamp,'mrro', [ROFROT(I,M)])
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'rofo_hh_t',timeStamp,'mrroo', [ROFOROT(I,M)]) !name?
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'rofs_hh_t',timeStamp,'mrros', [ROFSROT(I,M)]) !name?
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'rofb_hh_t',timeStamp,'mrrob', [ROFBROT(I,M)]) !name?
                    ...
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'evapotran_hh_t'   ,timeStamp,'evapotran', [EVAPSUM]) !name?
            ...
            if (ctem_on) then
                do m = 1,nmtest
                    ...
                    if (dopertileoutput) then
                        ...
                        call writeOutput1D(lonLocalIndex,latLocalIndex,'rml_hh_t',timeStamp,'rml', [rmlvegrow(I,M,:)])  !name
                    end if
                    anveggrd = 0.0
                    rmlveggrd = 0.0
                    do j = 1,icc
                    ...
                    call writeOutput1D(lonLocalIndex,latLocalIndex,'rml_hh',timeStamp,'rml', [rmlveggrd(:)])  !name
                end do
            end if
...
! ! 808                 CONTINUE
    ...
    subroutine class_monthly_aw(lonLocalIndex,latLocalIndex,IDAY,realyr,NCOUNT,NDAY,SBC,DELT,nltest,nmtest,TFREZ,&
        ...
        real, intent(in) :: DELT !CLASS common block items,
        real, intent(in) :: TFREZ !CLASS common block items,
        real, dimension(nlat,nmos), intent(in) :: ACTLYR          ! Active layer depth (m)  !FLAG why arguments?
        real, dimension(nlat,nmos), intent(in) :: FTABLE          ! Depth to frozen water table (m)  !FLAG why arguments?
...
subroutine ctem_daily_aw(nltest,nmtest,iday,FAREROT,realyr,jdstd,jdsty,jdendd,jdendy,grclarea,onetile_perPFT,ipeatlandrow)
    ...
    if ((realyr .ge. jdsty).and.(realyr.le.jdendy))then
...
90              continue !nmtest
                ...
                !>File: .CT01D
                write(72,8200)iday,realyr,gpp_g(i),npp_g(i), &
