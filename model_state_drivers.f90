...
contains
    ...
    subroutine read_initialstate()
        ...
        allocate(temp3d(nmos,cntx,cnty))

        call check_nc(nf90_inq_varid(initid,'DRN',varid))
        call check_nc(nf90_get_var(initid,varid,temp3d,start=[srtx,srty],count=[cntx,cnty]))
        DRNROT = map3d(temp3d,nlat,nmos)

        call check_nc(nf90_inq_varid(initid,'SDEP',varid))
        call check_nc(nf90_get_var(initid,varid,temp3d,start=[srtx,srty],count=[cntx,cnty]))
        SDEPROT = map3d(temp3d,nlat,nmos)
        !
        ...
        call check_nc(nf90_get_var(initid,varid,temp3d,start=[srtx,srty],count=[cntx,cnty]))
        XSLPROT = map3d(temp3d,nlat,nmos)

        call check_nc(nf90_inq_varid(initid,'GRKF',varid))
        call check_nc(nf90_get_var(initid,varid,temp3d,start=[srtx,srty],count=[cntx,cnty]))
        GRKFROT = map3d(temp3d,nlat,nmos)

        call check_nc(nf90_inq_varid(initid,'WFSF',varid))
        call check_nc(nf90_get_var(initid,varid,temp3d,start=[srtx,srty],count=[cntx,cnty]))
        WFSFROT = map3d(temp3d,nlat,nmos)

        call check_nc(nf90_inq_varid(initid,'WFCI',varid))
        call check_nc(nf90_get_var(initid,varid,temp3d,start=[srtx,srty],count=[cntx,cnty]))
        WFCIROT = map3d(temp3d,nlat,nmos)

        allocate(temp3di(nmos,cntx,cnty)) !this is an integer

        call check_nc(nf90_inq_varid(initid,'MID',varid))
        call check_nc(nf90_get_var(initid,varid,temp3di,start=[srtx,srty],count=[cntx,cnty]))
        MIDROT = map3di(temp3di,nlat,nmos)
    ...
    function map3di(threedin,dimone,dimtwo) result(out)
        use io_driver, only : cntx,cnty
        integer, intent(in) :: dimone,dimtwo
        integer, intent(in), dimension(:,:,:) :: threedin ! input
        integer, dimension(dimone,dimtwo)             :: out ! output
        integer :: x,y,k
        k = 0
        do x = 1, cntx
            do y = 1, cnty
    ...
    function map4d(fourdin,dimone,dimtwo,dimthree) result(out)
        use io_driver, only : cntx,cnty
        integer, intent(in) :: dimone,dimtwo,dimthree
        real, intent(in), dimension(:,:,:,:) :: fourdin ! input
        real, dimension(dimone,dimtwo,dimthree)     :: out ! output
        integer :: x,y,k,z
        k = 0
        do x = 1, cntx
            do y = 1, cnty
